...
| **Naver** | Supabase 가 기본 지원 안 함 → **Authentication → Providers → Custom** 으로 추가 |

### 4단계. URL 등록 (가장 자주 빠지는 단계)

좌측 **Authentication → URL Configuration**:

- **Site URL**: `http://localhost:3000` (초기) → 운영 도메인으로 갱신
- **Redirect URLs** (Add URL):
  - `http://localhost:3000/auth/callback`
  - 운영 도메인 등록 후엔: `https://white_eyes_matmap.com/auth/callback`, `https://xn--lo8h64a0d1a13a3lyqd.com/auth/callback` 모두 추가
...
| Name | Value | 비고 |
|---|---|---|
| `NEXT_PUBLIC_API_BASE_URL` | `https://your-fastapi-host` | FastAPI 배포 호스트 |
| `NEXT_PUBLIC_SUPABASE_URL` | `https://xxx.supabase.co` | |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | `eyJ...` | |
...
4. **Environment** 탭에서 시크릿 등록:
   ...
   ```
5. Deploy → URL 확인 (예: `https://baekahn-matjido-api.onrender.com`)
6. Vercel 의 `NEXT_PUBLIC_API_BASE_URL` 갱신 → Redeploy
7. 헬스체크: `curl https://baekahn-matjido-api.onrender.com/healthz`
...
[도메인 운영](#-도메인-운영-가이드) 완료 후:

| 콘솔 | 등록 위치 | 등록 값 |
|---|---|---|
| **Kakao Developers** | 내 애플리케이션 → 플랫폼 → **Web** → 사이트 도메인 | 두 도메인 모두 |
...
| **Supabase** | Authentication → URL Configuration | Site URL = 주 도메인, Redirect URLs 에 두 도메인의 `/auth/callback` |
...
> 한글 도메인은 Punycode 로 `xn--lo8h64a0d1a13a3lyqd.com` 로 표기됨 — 브라우저 주소창에는 한글 그대로.
...
| Cloudflare Tunnel | 0원 | 자체 PC 운영 시 |
...
### 영문 — `white_eyes_matmap.com`

**Porkbun** (영문, 카드결제, 약 $11/년 ≈ 15,000원, WHOIS 무료):

1. https://porkbun.com → Sign up
2. `white_eyes_matmap.com` 검색 → Add to cart
3. WHOIS Privacy 자동 적용
4. Auto-renew: ON
5. 결제

또는 **가비아** (한국 카드/계좌, 약 17,000원/년).

### 한글 — `백안맛지도.com`

**가비아** 추천 — 한글 입력 인터페이스 직관:

1. https://www.gabia.com 도메인 검색창에 `백안맛지도.com` 입력
2. Punycode (`xn--lo8h64a0d1a13a3lyqd.com`) 확인 후 등록
3. 한국 카드/계좌로 결제

> Porkbun 등 영문 등록자도 가능. Punycode 변환: https://www.punycoder.com 에 `백안맛지도.com` 입력.

## 2단계. Vercel 에 도메인 연결

1. Vercel 대시보드 → 프로젝트 선택 → **Settings → Domains**
2. **Add Domain** → `white_eyes_matmap.com` → Add
   - Vercel 이 표시하는 DNS 레코드 (A 레코드 `76.76.21.21` 또는 nameserver) 메모
3. **Add Domain** → `xn--lo8h64a0d1a13a3lyqd.com` → Add (한글 도메인은 Punycode)
4. (선택) **Redirect** — 한쪽을 주 도메인으로:
   - 예: `xn--lo8h64a0d1a13a3lyqd.com` → `white_eyes_matmap.com` 301 redirect

## 3단계. DNS 레코드 설정

도메인 등록자(가비아/Porkbun)의 DNS 관리 페이지에서:
...
> Vercel 의 "Configure DNS" 가 표시하는 정확한 값을 따르세요.

### (선택) Cloudflare DNS — CDN/보안/분석 무료

1. https://dash.cloudflare.com → Add a Site → 도메인 입력
2. Cloudflare 가 알려주는 2개의 nameserver 메모
3. 도메인 등록자 → DNS 관리 → **nameserver 변경** → Cloudflare nameserver
4. Cloudflare DNS 에서 A 레코드 → Vercel IP + Proxy status: **DNS only**
5. nameserver 전파 ~수시간

## 4단계. SSL 인증서

Vercel 자동 발급 (Let's Encrypt). 도메인 추가 후 1-5분 내 자동 활성. 별도 작업 없음.

## 5단계. 운영 도메인을 모든 외부 콘솔에 등록

[Vercel 5단계](#5단계-외부-콘솔에-운영-도메인-등록) 표 참고. **반드시 두 도메인 모두 등록.**

## 6단계. 헬스체크

| 항목 | 확인 |
|---|---|
| 영문 도메인 | `https://white_eyes_matmap.com/about` 정상 렌더, SSL 자물쇠 |
| 한글 도메인 | 브라우저 주소창에 `백안맛지도.com` 입력 → 정상 렌더 |
| API | `/healthz` → `{"ok": true}` |
| 지도 | `/` 진입 후 카카오 지도 로드. 회색이면 Kakao 콘솔 도메인 등록 누락 |
| 로그인 | `/auth/login` → OAuth 콜백 성공 (두 도메인 모두) |
...
가장 보편적. 한국 거주자도 신청·수익화 가능. 단 **승인 절차** 가 까다로움 (콘텐츠 충분 + 도메인 등록 + 1~4주 검수).
...
> 한글 도메인 `백안맛지도.com` 도 같은 AdSense 계정에서 **사이트 추가** 로 별도 등록.
> 두 사이트 모두 승인 받아야 광고 노출.
...
AdSense 가 요구하는 신뢰 파일. 두 도메인 모두 `/ads.txt` 로 서빙되어야 함.
...
> Vercel 의 public/ 은 자동으로 정적 서빙 → `https://white_eyes_matmap.com/ads.txt` 접근 가능.
