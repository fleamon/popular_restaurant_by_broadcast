...
| `visits.get_stats()` distinct | 전체 `visitor_id` 를 메모리 set 으로 unique 계산 → 데이터 폭증 시 OOM. `count(distinct)` RPC/뷰로 이전 | 🔴 스케일 시 필수(DB) |
| `dislikes` 완전 제거 | UI 제거 완료, 백엔드 컬럼/타입 잔존(레거시). DB 컬럼 DROP + 타입 정리 | 🟡 중간(DB·데이터 손실) |
