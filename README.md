...
- ✅ **상세 페이지 SSR/메타데이터 + 동적 OG** (구현) — `generateMetadata`(가게별 title/description/canonical) + `opengraph-image.tsx`. 상호작용 UI 는 `RestaurantDetailClient` 로 분리.
...
**Google AdSense 콘솔**
- 사이트 검토 **재요청**(콘텐츠/광고 정책 수정 반영). 등록 도메인 = `xn--0z2byb.com`(ads.txt 200 확인됨).
