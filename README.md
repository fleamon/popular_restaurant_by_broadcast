...
- **상세 페이지 SSR/메타데이터** — `/restaurants/[id]` 서버 컴포넌트화 + 가게별 동적 메타/OG. (대규모 리팩터)
- **검색 결과 URL 공유 OG** — 필터 조합별 동적 OG 이미지. (`@vercel/og` 인프라)
- **요청 게시판 알림** — 답변 시 메일 알림. (Supabase Edge Function + 메일 발송자)
- **다국어(i18n)** — 영어 지원. (전면 번역 — 대규모)
