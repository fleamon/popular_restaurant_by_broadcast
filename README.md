...
4. 운영 중인 DB 는 새 파일만 차례로: `0006_daily_votes.sql` → `0007_request_restaurant_edit.sql` → `0008_reject_pending_restaurant_requests.sql` → `0009_visits.sql` → `0010_bookmarks.sql` → `0011_visits_referer.sql`(방문 유입 출처) → `0012_visitors_count_rpc.sql`(누적 방문자 RPC) → `0013_remove_dislikes.sql`(싫어요 제거 — **value=-1 영구 삭제**)
...
- **콘텐츠 보강**:
  - 홈(`/`) — 서버 렌더 `HomeIntro`: 서비스 소개 + 이용 방법(4단계) + 자주 묻는 질문(FAQ) + 지역/카테고리 빠른 필터 칩(내부 링크). 크롤러가 받는 초기 HTML 에 원문 텍스트가 담긴다.
  - 맛집 상세 — `RestaurantSummary` 가 채널·카테고리·위치로 한 줄 소개를 생성. YouTube 는 공식 임베드 + 원본 링크. 본문 전체가 **서버 렌더(SSR)** 되어 HTML 에 콘텐츠가 존재.
- 재검토 요청 전 체크: 충분한 맛집 데이터(상세 페이지 다수)·고유 콘텐츠·정상 동작 페이지. 준비 중/빈 페이지 없도록.

#### 2026-06 거절("가치가 별로 없는 콘텐츠") 대응 — 근본 원인·수정

거절 시점의 핵심 원인은 **SPA 의 클라이언트 렌더**였다. 크롤러/AdSense 가 받는 초기 HTML 이 거의 비어 있어(본문은 JS 실행 후에야 채워짐) 사이트 전체가 'thin' 으로 평가됐다.

| # | 근본 원인 | 수정 | 위치 |
|---|---|---|---|
| 1 | **맛집 상세 본문이 전부 클라이언트 렌더** — 크롤러가 받는 HTML 은 `불러오는 중…` 뿐. 사이트 콘텐츠의 대부분인 상세 페이지가 빈 페이지로 보임 | `page.tsx` 가 서버에서 식당·영상·연관맛집을 fetch 해 **초기 props 로 SSR**. `RestaurantDetailClient` 는 그 props 로 state 를 seed → HTML 에 실제 본문(이름·주소·소개·영상 제목·채널·연관 맛집)이 담기고 로딩 깜빡임도 사라짐 | `restaurants/[id]/page.tsx`, `RestaurantDetailClient.tsx` |
| 2 | **sitemap 에 상세 페이지가 하나도 없음** — 정적 6페이지만 등록 → '6페이지짜리 빈약한 사이트'로 보임 | `sitemap.ts` 가 백엔드 경량 엔드포인트(`GET /restaurants/sitemap`, id+updated_at)에서 **모든 맛집 상세 URL 을 동적 생성**(1h ISR). 폐업 맛집 제외 | `app/web/src/app/sitemap.ts`, `routers/restaurants.py` |
| 3 | **홈 본문이 전부 클라이언트 렌더** — `use client` + `useSearchParams` + 레이아웃의 `Suspense fallback={null}` 로 정적 HTML 에 본문이 통째로 비어 있었음(19KB → 텍스트 0) | 홈을 **서버 컴포넌트**로 분리: 검색 UI(`HomeSearch`, 클라이언트)는 자체 `Suspense` 로 감싸 bail-out 을 가두고, `HomeIntro`(이용방법·FAQ·내부 링크)는 서버 렌더 → 정적 HTML 에 본문 포함(48KB) | `app/web/src/app/page.tsx`, `HomeSearch.tsx`, `HomeIntro.tsx` |
| 4 | **홈→상세 크롤 경로 부재** — 홈의 맛집 목록/지도는 클라이언트 렌더라 서버 HTML 에 상세(`/restaurants/{id}`)로 가는 내부 링크가 0개. sitemap 으로만 발견되는 페이지는 신뢰도가 낮음 | 홈에 **`PopularRestaurants`(서버 렌더, 좋아요 순 상위 40개)** 추가 — 각 카드가 상세 페이지로 가는 내부 링크. 검색엔진/AdSense 가 진입점에서 상세를 따라가 발견·평가 + 홈 콘텐츠도 보강 | `app/web/src/components/PopularRestaurants.tsx`, `routers/restaurants.py`(`/top`) |
| 5 | **구조화 데이터 부재** — 검색엔진에 페이지 주제·사이트 정체성을 알리는 신호 없음 | JSON-LD 추가 — 사이트 전역 `WebSite`(+검색 SearchAction), 홈 `FAQPage`, 맛집 상세 `Restaurant`(+`VideoObject`) | `layout.tsx`, `HomeIntro.tsx`, `restaurants/[id]/page.tsx` |

> **AdSense 는 sitemap 으로 콘텐츠를 판단하지 않는다.** sitemap 은 URL '발견'용이고, 실제 판단은 크롤러가 각 페이지를 **렌더링해 본문이 충분한지** 본다. 따라서 ①페이지 HTML 에 콘텐츠가 있을 것(SSR), ②sitemap 에 등록될 것, ③내부 링크로 도달 가능할 것 — 셋 다 필요하다. 위 1·3·4 가 각각 대응한다.
>
> 검증: `cd app/web && npm run build` 후 `.next/server/app/index.html` 에 홈 본문(이용 방법·FAQ)과 `href="/restaurants/{id}"` 링크가 포함되는지, `sitemap.xml.body` 에 상세 URL 이 포함되는지 grep 으로 확인. 상세 페이지는 `ƒ (Dynamic)` 로 빌드되어 요청 시 본문이 SSR 됨. (로컬은 API 가 꺼져 있으면 홈 인기목록·sitemap 상세가 비므로, 운영 API 기준으로 확인)
>
> **재검토 전 운영 확인**: ① 백엔드 배포 후 `curl https://<api>/restaurants/sitemap` 200 + 배열 응답, ② `https://<도메인>/sitemap.xml` 에 `/restaurants/<id>` URL 들이 포함, ③ 상세 페이지 '소스 보기'(JS 끈 상태)에서 가게명·주소·소개 텍스트가 보이는지. 그 후 AdSense 콘솔에서 **사이트 검토 재요청**.
...
- ✅ `in_chunks` 공통 헬퍼(`supabase_client`) 추출 → channels·restaurants 중복 청크 로직 통일.
- ✅ `VoteState` 타입 `lib/api.ts` 로 중앙화 → Map·상세·mypage 중복 정의 제거.
...
**처리 완료(추가)**
- ✅ `visits.get_stats()` 누적 방문자 → `count_distinct_visitors()` RPC(`0012`)로 이전 — 메모리 distinct 제거.
- ✅ `dislikes` 제거 — `value=-1` 삭제 + `value=1` 제약(`0013`), `VoteRequest.value=Literal[1]`. 랭킹 뷰는 안정성 위해 유지(dislikes 항상 0, 미노출). 향후 뷰/응답에서 컬럼까지 빼는 건 별도.

## 💡 향후 기능 아이디어

- ✅ **'근처 맛집' / 연관 추천** (구현) — 상세에 같은 시군구(우선)·카테고리 다른 맛집 링크. `/restaurants/{id}/related`.
- ✅ **맛집 설명(notes) 입력 UI** (구현) — admin `RestaurantManagement` 메모를 textarea 로 확장 → 상세 페이지 고유 콘텐츠.
- ✅ **상세 지도 미리보기** (구현) — 좌표 있는 맛집은 상세에 단일 핀 지도(기존 `Map` 재사용).
- ✅ **방문자 유입 그룹화** (구현) — `VisitorChart` 가 referer 를 검색/SNS·영상/직접/사이트내/기타 그룹으로 요약.
- ✅ **상세 페이지 SSR/메타데이터 + 동적 OG** (구현) — `generateMetadata`(가게별 title/description/canonical) + `opengraph-image.tsx`. **본문도 서버에서 fetch 해 초기 props 로 SSR**(크롤러가 빈 페이지가 아닌 실제 콘텐츠를 받음). 상호작용 UI 는 `RestaurantDetailClient` 로 분리.
- ✅ **동적 sitemap + 홈 크롤 경로 + 구조화 데이터(JSON-LD)** (구현) — `sitemap.ts` 가 모든 맛집 상세 URL 동적 포함(`GET /restaurants/sitemap`), 홈을 서버 컴포넌트로 분리해 `PopularRestaurants`(인기 맛집 → 상세 내부 링크)·`HomeIntro`(이용방법·FAQ) 서버 렌더, `WebSite`/`FAQPage`/`Restaurant`+`VideoObject` JSON-LD. → AdSense '저가치 콘텐츠' 거절 대응.
- **검색 결과 URL 공유 OG** — 필터 조합별 동적 OG 이미지. (`next/og` 라우트 추가로 확장 가능)
- **요청 게시판 알림** — 보류. 메일 발송 서비스 미보유 + 현재 불필요. (필요 시 인앱 알림 배지로 대체 가능)
- **다국어(i18n)** — 보류(권장하지 않음). 한국 방송 맛집 데이터·한국 타깃이라 ROI 낮고, 데이터(상호·주소)는 번역 불가. 부분 번역은 UI 혼재로 UX 악화 → 한다면 `next-intl` 기반 전면 번역의 별도 프로젝트로.

> 진행 시 항목별로 범위·DB 영향을 다시 협의한다. SSR·OG·이메일·i18n 은 인프라/대규모라 단계적으로.

---

## ✅ 운영 수동 작업 체크리스트

코드 push 만으로는 반영되지 않고 **운영 콘솔에서 직접** 해야 하는 것들. (이미 했으면 스킵)

**Supabase SQL Editor — 마이그레이션 실행** (새 파일만 순서대로)
- `0011_visits_referer.sql` — 방문 유입 출처 컬럼.
- `0012_visitors_count_rpc.sql` — 누적 방문자 RPC. 미실행 시 `/mypage` 방문자 그래프의 **총 방문자수가 0** 으로 표시됨.
- `0013_remove_dislikes.sql` — 싫어요 제거. **`value=-1` 행이 영구 삭제**되고 이후 `value=1` 만 허용. (미실행이어도 UI 동작엔 문제 없음 — 단 옛 싫어요 데이터가 점수 뷰에 남아있을 뿐)

**Render (백엔드)**
- Start Command: `gunicorn app.api.main:app -k uvicorn.workers.UvicornWorker -w 1 --max-requests 2000 --max-requests-jitter 200 --timeout 120 --bind 0.0.0.0:$PORT` (메모리 OOM 예방). `.python-version`(3.12.7)은 repo 에 있어 자동 적용.

**Vercel (웹) — canonical 도메인 통일** ⚠️
- 운영 도메인은 **apex `xn--0z2byb.com`** 이 canonical(`www` 는 308 → apex 리다이렉트, AdSense 등록 도메인도 apex).
- Vercel → Settings → Environment Variables 의 **`NEXT_PUBLIC_SITE_URL` 을 `https://xn--0z2byb.com`** (www 없이)로 설정 후 Redeploy. 비워두면 코드 기본값(apex)이 적용됨.
  - 이게 `www` 로 박혀 있으면 sitemap 의 모든 URL·canonical·og:url 이 `www` 가 되어 **5천여 개 URL 이 전부 리다이렉트**된다(검색엔진이 "sitemap 에 리다이렉트 포함" 으로 감점 가능).

**Google AdSense 콘솔**
- 사이트 검토 **재요청**(2026-06 "가치가 별로 없는 콘텐츠" 거절 → SSR·sitemap·콘텐츠 보강 반영 후). 등록 도메인 = `xn--0z2byb.com`(ads.txt 200 확인됨).
- 재요청 전 확인: 상세 페이지가 JS 없이도 본문 노출(SSR), `/sitemap.xml` 에 맛집 상세 URL 포함, 백엔드 `/restaurants/sitemap` 200. (위 "광고 붙이기 → AdSense 승인·콘텐츠 품질 대응 → 2026-06 거절 대응" 표 참조)
- Google Search Console 에 `/sitemap.xml` 제출 → 색인 가속(권장).
- EU 트래픽 대비 **개인정보 보호 및 메시지 → GDPR 메시지(CMP)** 게시(선택).
- `NEXT_PUBLIC_ADSENSE_CLIENT` 환경변수 유지(비우면 ads.txt 404·광고 미로드).
